...
for i=1:length(x)
    ...
    if col~=0 && ~isnan(vect(i))
        p=patch(xhex+x(i),yhex+y(i),vect(i),'EdgeColor','none'); % make a hexagon at [2i,2j]
        tx=text(x(i), y(i), num2str(round(vect(i),2)),'HorizontalAlignment','center','VerticalAlignment','middle','Color',c(col,:),'FontUnits', 'Normalized','FontSize',2e-2);
        hold on
    end
end
try
