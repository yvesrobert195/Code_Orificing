function P=make_constraints_tolerance(P,I,C,G)
nass=P.Var.nass;
...
nineqs = nass*nsteps + 2*nsteps + nass*nsteps + 2*nadj*nsteps + npossflows*nass + nsteps*nass+2*nass+nass; %number of inequality constraints
neqs = nass; %number of equality constraints

%initialize constraint matrices and vectors
nelements_Aineq = nass*nsteps*npossflows+ 2*nsteps*nass*npossflows + nass*nsteps + 2*nsteps*nadj*npossflows + nass*npossflows + nsteps*nass*npossflows+2*(npossflows+1)*nass+nass;
Aineq_i = zeros(nelements_Aineq,1);
Aineq_j = zeros(nelements_Aineq,1);
Aineq_v = zeros(nelements_Aineq,1);
bineq = zeros(nineqs, 1);
nelements_Aeq = npossflows*nass;
...
for i = 1:nass
    Aineq_i(idx_Aineq) = nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+nass*npossflows+nass*nsteps+i;
    Aineq_j(idx_Aineq) = i;
    Aineq_v(idx_Aineq) = -1;
    idx_Aineq = idx_Aineq+1;
    for j = 1:npossflows
        Aineq_i(idx_Aineq) = nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+nass*npossflows+nass*nsteps+i;
        Aineq_j(idx_Aineq) = nass+(i-1)*npossflows+j;
%         Aineq_v(idx_Aineq) = (1-P.Constraints.tol)*x(j);
        Aineq_v(idx_Aineq) = x(j);
        idx_Aineq = idx_Aineq+1;
    end
    bineq(nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+nass*npossflows+nass*nsteps+i) = P.Constraints.tol;
    
    Aineq_i(idx_Aineq) = nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+nass*npossflows+nass*nsteps+nass+i;
    Aineq_j(idx_Aineq) = i;
    Aineq_v(idx_Aineq) = 1;
    idx_Aineq = idx_Aineq+1;
    for j = 1:npossflows
...
for i=1:nass
     Aineq_i(idx_Aineq) = nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+nass*npossflows+nass*nsteps+2*nass+i;
    Aineq_j(idx_Aineq) = i;
    Aineq_v(idx_Aineq) = -1;
    idx_Aineq = idx_Aineq+1;
     bineq(nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+nass*npossflows+nass*nsteps+2*nass+i) = 0;
end
%% EQUALITIES
