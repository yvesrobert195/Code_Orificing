function P=constraints_xi(P,I,C,G)
nass=P.Var.nass;
...
nineqs = nass*nsteps + 2*nsteps + nass*nsteps + 2*nadj*nsteps + nsteps*nass; %number of inequality constraints
neqs = nass + nass; %number of equality constraints

%initialize constraint matrices and vectors
nelements_Aineq = nass*nsteps*npossflows+ 2*nsteps*nass*npossflows + nass*nsteps + nsteps*(nadj*(npossflows*2+1))*2 + nsteps*nass*npossflows;
    ...
    for i = 1:nass
        for ip = I.adjacentAssemblies(i,:)
            if ip > 0
                ...
                Aineq_i(idx_Aineq)= nsteps*nass+2*nsteps+nsteps*nass+constraint_idx;
                Aineq_j(idx_Aineq)= nass+nass*npossflows+1;
                if G.rings(i)<=G.nrings && G.rings(ip)<=G.nrings
                    Aineq_v(idx_Aineq)= -1;
                    bineq(nsteps*nass+2*nsteps+nsteps*nass+constraint_idx)=0;
                else
                    Aineq_v(idx_Aineq)= 0;
...
for k=1:nsteps
    for i = 1:nass
        for j = 1:npossflows
            Aineq_i(idx_Aineq) = nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+(k-1)*nass+i;
            Aineq_j(idx_Aineq) = nass+(i-1)*npossflows+j;
...
bineq(nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+1:nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+nass*nsteps) = P.Constraints.dP_max;
...
P.CPLEX.c(nass+nass*npossflows+1) = 1;
