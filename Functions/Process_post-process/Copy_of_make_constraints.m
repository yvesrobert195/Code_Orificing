function P=Copy_of_make_constraints(P,I,C,G)
nass=P.Var.nass;
...
nineqs = nass*nsteps + 2*nsteps + nass*nsteps + 2*nadj*nsteps + npossflows*nass + nsteps*nass+1 + 1; %number of inequality constraints
neqs = nass + nass; %number of equality constraints

%initialize constraint matrices and vectors
nelements_Aineq = nass*nsteps*npossflows+ 2*nsteps*nass*npossflows + nass*nsteps + 2*nsteps*nadj*npossflows + nass*npossflows + nsteps*nass*npossflows+npossflows+1 +1;
...
for i = 1:nvars
    ...
    elseif i< nvars %beta variables
        P.CPLEX.ctype(end+1) = 'C';
    elseif i==nvars
        P.CPLEX.ctype(end+1) = 'I';
    end
end
...
% B>=sum(Bj)
for j = 1:npossflows
    Aineq_i(idx_Aineq) = nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+nass*npossflows+nass*nsteps+1;
    Aineq_j(idx_Aineq) = nass+nass*npossflows+j;
    Aineq_v(idx_Aineq) = 1;
    idx_Aineq = idx_Aineq+1;
end
Aineq_i(idx_Aineq) = nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+nass*npossflows+nass*nsteps+1;
Aineq_j(idx_Aineq) = nass+nass*npossflows+npossflows+1;
Aineq_v(idx_Aineq) = -1;
idx_Aineq = idx_Aineq+1;
bineq(nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+nass*npossflows+nass*nsteps+1)=0;

Aineq_i(idx_Aineq) = nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+nass*npossflows+nass*nsteps+2;
Aineq_j(idx_Aineq) = nass+nass*npossflows+npossflows+1;
Aineq_v(idx_Aineq) = 1;
bineq(nsteps*nass+2*nsteps+nsteps*nass+2*nadj*nsteps+nass*npossflows+nass*nsteps+2)=21;
idx_Aineq = idx_Aineq+1;
...
P.CPLEX.beq=beq;
%objective
P.CPLEX.c(nass+nass*npossflows+npossflows+1) = 1;
